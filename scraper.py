...
def scraper_by_ticker_from_yahoo(ticker, maxCards):
    ...
    news_data_lst = []
    for url in url_lst:
        print(f"process page #{i} out of {maxCards}")
...
def main():
    ...
    for ticker_name in ticker_name_lst:
        news_data = scraper_by_ticker_from_yahoo(ticker_name, 50)
        if not news_data:
